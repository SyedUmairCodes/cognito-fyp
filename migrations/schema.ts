...
export const users = pgTable(
  ...
  (table) => {
    return {
      ...
      canUpdateOwnUserData: pgPolicy("Can update own user data.", {
        as: "permissive",
        for: "update",
        to: ["public"],
        using: sql`(( SELECT auth.uid() AS uid) = id)`,
      }),
      everyoneCanViewUserData: pgPolicy("Everyone can view user data.", {
        as: "permissive",
        for: "select",
        to: ["public"],
      }),
...
export const products = pgTable(
  ...
  (table) => {
    return {
      allowPublicReadOnlyAccess: pgPolicy("Allow public read-only access.", {
        as: "permissive",
        for: "select",
        to: ["public"],
        using: sql`true`,
...
export const subscriptions = pgTable(
  ...
  (table) => {
    return {
      ...
      canOnlyViewOwnSubsData: pgPolicy("Can only view own subs data.", {
        as: "permissive",
        for: "select",
        to: ["public"],
